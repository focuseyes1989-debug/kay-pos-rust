...
pub fn CategoriesPage(db_form: DbForm, on_sales: EventHandler<()>) -> Element {
    ...
    rsx! {
        section {class:"customers_page products_touch categories_touch",
            header {class:"customers_header customers_touch_header",h2 {"Categories"}
                div {class:"customers_actions",
                    button {disabled:!data.finished(),onclick:move |_|data.restart(),"Refresh"}
                }
            }
            section {class:"products_items",
                header {class:"products_items_header",div {class:"category_heading",h3 {"Categories"}
                    ...
                    button {class:"customer_primary",disabled:!data.finished(),onclick:move |_|editor.set(Some(CategoryRecord {status:"active".into(),..Default::default()})),"+ Add category"}
                }
                    ...
                    if !data.finished() {div {class:"product_loading",role:"status",span {class:"loading_spinner"} strong {"Loading..."}}}
                        ...
                        article {key:"{row.id}",class:match depth {0=>"category_row category_parent",1=>"category_row category_child",_=>"category_row category_subchild"},style:"margin-left:{depth.min(5)*20}px",
                            ...
                            div {class:"customers_actions",
                                ...
                                button {disabled:row.is_system!=0,onclick:{let row=row.clone();move |_|editor.set(Some(row.clone()))},"Edit"}
                                button {class:"product_delete",disabled:row.is_system!=0,onclick:move |_|deletion.set(Some(row.clone())),"Delete"}
                            }
                        }
                    }}
                }
        ...
        if let Some(record)=deletion() {
            div {class:"modal_backdrop",section {class:"customer_dialog",role:"dialog",aria_modal:"true",aria_label:"Delete category",h2 {"Delete category?"} p {"{record.name}"}
                div {class:"customers_actions",button {disabled:busy(),onclick:move |_|deletion.set(None),"Cancel"}
                    button {class:"product_delete",disabled:busy(),onclick:move |_|{if busy(){return;}busy.set(true);let source=db_form.clone();spawn(async move {let result=async {categories::delete(&connect(&source.database_config()?).await?,record.id).await}.await;busy.set(false);match result {Ok(())=>{deletion.set(None);data.restart();},Err(e)=>error.set(format!("{e:#}"))}});},"Delete"}
                }
            }}
        }
...
fn CategoryEditor(
    ...
    rsx! {div {class:"modal_backdrop",section {class:"customer_dialog",role:"dialog",aria_modal:"true",aria_label:"Category",
        ...
        div {class:"customers_actions",
            button {disabled:busy(),onclick:move |_|on_close.call(()),"Cancel"}
